...
public class Uni5Exe27 {
  ...
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if (dia < 0 || dia > 31) {
        ...
        if(producaoManha > producaoTarde) {
          System.out.println("A produção da manhã foi maior que a da tarde; " + producaoManha + " produtos produzidos." );
        }
        if(producaoTarde > producaoManha) {
          System.out.println("A produção da tarde foi maior que a produção da manhã; " + producaoTarde + " produtos produzidos.");
        }
        ...
        if(opt == 2) {
          System.out.println("Resultados de desempenho obtidos e sistema finalizado.");
          break;
        }
    ...
    int maxIndex = producao.indexOf(producao.stream().max(Comparator.naturalOrder()).get());

    System.out.println("O dia onde obteve-se a maior produção foi o dia: " + dias.get(maxIndex) + " de Abril.");
